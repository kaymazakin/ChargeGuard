...
const outputApk = path.join(root, "ChargeGuard.apk");
...
const compiledResources = collectFiles(compiledResDir, ".flat");
const linkArgs = [
  "link",
  "-o", baseApk,
  "-I", androidJar,
...
const javaFiles = [
  ...collectFiles(path.join(root, "src"), ".java"),
  ...collectFiles(genDir, ".java"),
];
...
const classFiles = collectFiles(classesDir, ".class");
run("java", [
  "-cp", d8Jar,
...
if (!fs.existsSync(keystore)) {
  run("java", [
    ...
    "-keysize", "2048",
    "-validity", "10000",
    "-dname", "CN=Codex Debug,O=ChargeGuard,C=TR",
  ]);
}
